...
public class GDEItemManagerWindow : GDEManagerWindowBase
{
    ...
    void DrawSingleField(string schemaKey, string itemKey, string fieldKey, Dictionary<string, object> itemData)
    {
        ...
        {
            DrawObject<VideoClip>(itemKey, fieldPreviewKey, fieldKey, itemData, GDEConstants.ValueLbl);
            drawHelper.NewLine();
            break;
        }
        default:
    ...
    void DrawListField(string schemaKey, string itemKey, string fieldKey, Dictionary<string, object> itemData)
    {
        try
        {
            ...
            if (runtimeValues)
            {
                ...
                case BasicFieldType.VideoClip:
                {
                    rtList = GDEDataManager.GetUnityObjectList<VideoClip>(itemKey, fieldKey);
                    if (rtList == null)
                        rtList = (temp as IList).Cast<VideoClip>().ToList();
                    break;
                }
                case BasicFieldType.String:
                default:
            ...
            if (newFoldoutState)
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    case BasicFieldType.VideoClip:
                    {
                        needsSave |= DrawListObject<VideoClip>(itemKey, fieldKey, foldoutKey + i, content, i, list[i] as VideoClip, list);
                        drawHelper.NewLine();
                        break;
                    }
                    default:
    ...
    void Draw2DListField(string schemaKey, string itemKey, string fieldKey, Dictionary<string, object> itemData)
    {
        try
        {
            ...
            if (runtimeValues)
            {
                ...
                case BasicFieldType.VideoClip:
                {
                    rtList = GDEDataManager.GetUnityObjectTwoDList<VideoClip>(itemKey, fieldKey);
                    if (rtList == null)
                        rtList = list.Cast<List<VideoClip>>().ToList();
                    break;
                }
                case BasicFieldType.String:
                default:
                ...
                for (int index = 0; index < list.Count; index++)
                {
                    ...
                    if (isOpen)
                    {
                        for (int x = 0; x < subList.Count; x++)
                        {
                            ...
                            case BasicFieldType.VideoClip:
                            {
                                needsSave |= DrawListObject<VideoClip>(itemKey, fieldKey, foldoutKey + index + "_" + x, content, x, subList[x] as VideoClip, subList);
                                drawHelper.NewLine();
                                break;
                            }
                            default:
    ...
    List<string> GetPossibleCustomValues(string fieldKey, string fieldType)
    {
        ...
        if (!runtimeValues)
        {
            // Determine if this is a custom enum or not
            if (GDEItemManager.EnumTypes.Contains(fieldType))
            {
                itemKeys.Clear();
                itemKeys.AddRange(GDEItemManager.EnumValues[fieldType]);
            }
            else
            {
    ...
    protected override void Load(bool forceLoad = false)
    {
        base.Load(forceLoad);
    ...
    void SetRuntimeListValue(string item, string field, IList val, BasicFieldType type, int dimension = 0)
    {
        ...
        case BasicFieldType.VideoClip:
        {
            if (dimension == 1)
            {
                List<VideoClip> tmp = val as List<VideoClip>;
                GDEDataManager.SetUnityObjectList(item, field, tmp);
            }
            else if (dimension == 2)
            {
                List<List<VideoClip>> tmp = val as List<List<VideoClip>>;
                GDEDataManager.SetUnityObjectTwoDList(item, field, tmp);
            }
            break;
        }
