...
public class GDECodeGen
{
	...
	static void AppendVariableDeclarations(StringBuilder sb, string schemaKey, Dictionary<string, object> schemaData)
	{
		...
		foreach (string fieldKey in GDEItemManager.SchemaCustomFieldKeys(schemaKey, 0))
		{
			...
			// Only append data class format to custom types not custom enums
			isEnum = GDEItemManager.EnumTypes.Contains(variableType);
			if (!isEnum)
				variableType = string.Format(GDECodeGenConstants.DataClassNameFormat, variableType);
			AppendCustomVariable(sb, variableType, fieldKey, isEnum);
		...
		for (int dimension = 1;  dimension <= 2;  dimension++)
		{
			foreach (string fieldKey in GDEItemManager.SchemaCustomFieldKeys(schemaKey, dimension))
			{
				...
				AppendCustomListVariable(sb, variableType, fieldKey, dimension, isEnum);
		...
		for (int dimension = 0;  dimension <= 2;  dimension++)
		{
			foreach (BasicFieldType fieldType in GDEItemManager.BasicFieldTypes)
			{
				...
				foreach (string fieldKey in fieldKeys)
				{
					...
					AppendSaveToDictVariable(sb, schemaKey, fieldKey, dimension);
					shouldAppendSpace = true;
					didAppendSpaceForSection = true;
					isFirstSection = false;
				}
	...
	static void AppendUpdateCustomItemsMethod(StringBuilder sb, string schemaKey, Dictionary<string, object> schemaData)
	{
		...
		for (int dimension = 0;  dimension <= 2;  dimension++)
		{
			foreach (string fieldKey in GDEItemManager.SchemaCustomFieldKeys(schemaKey, dimension))
			{
				if (GDEItemManager.IsEnum(schemaKey, fieldKey))
					continue;
	...
	static void AppendLoadDictMethod(StringBuilder sb, string schemaKey, Dictionary<string, object> schemaData)
	{
		...
		foreach (string fieldKey in GDEItemManager.SchemaCustomFieldKeys(schemaKey, 0))
		{
			...
			if (appendTempKeyDeclaration && !isEnum)
			{
			...
			AppendLoadCustomVariable(sb, variableType, fieldKey, isEnum);
			shouldAppendSpace = true;
			didAppendSpaceForSection = true;
			isFirstSection = false;
		}
		...
		for (int dimension = 1;  dimension <= 2;  dimension++)
		{
			foreach (BasicFieldType fieldType in GDEItemManager.BasicFieldTypes)
			{
				...
				foreach (string fieldKey in fieldKeys)
				{
					...
					AppendLoadListVariable(sb, variableType, fieldKey, dimension, false);
					shouldAppendSpace = true;
					didAppendSpaceForSection = true;
					isFirstSection = false;
				}
		...
		for (int dimension = 1;  dimension <= 2;  dimension++)
		{
			foreach (string fieldKey in GDEItemManager.SchemaCustomFieldKeys(schemaKey, dimension))
			{
				...
				AppendLoadListVariable(sb, variableType, fieldKey, dimension, isEnum);
	...
	static void AppendLoadFromSavedMethod(StringBuilder sb, string schemaKey, Dictionary<string, object> schemaData)
	{
		...
		foreach (string fieldKey in GDEItemManager.SchemaCustomFieldKeys(schemaKey, 0))
		{
			...
			AppendLoadFromSavedCustomVariable(sb, variableType, fieldKey, isEnum);
			shouldAppendSpace = true;
			didAppendSpaceForSection = true;
			isFirstSection = false;
		}
		...
		for (int dimension = 1;  dimension <= 2;  dimension++)
		{
			foreach (BasicFieldType fieldType in GDEItemManager.BasicFieldTypes)
			{
				...
				foreach (string fieldKey in fieldKeys)
				{
					...
					AppendLoadSavedListVariable(sb, variableType, fieldKey, dimension, false);
					shouldAppendSpace = true;
					didAppendSpaceForSection = true;
					isFirstSection = false;
				}
		...
		for (int dimension = 1;  dimension <= 2;  dimension++)
		{
			foreach (string fieldKey in GDEItemManager.SchemaCustomFieldKeys(schemaKey, dimension))
			{
				...
				if (!isEnum)
					variableType = "Custom";
				AppendLoadSavedListVariable(sb, variableType, fieldKey, dimension, isEnum);
	...
	static void AppendResetVariableMethods(StringBuilder sb, string schemaKey, Dictionary<string, object> schemaData)
	{
		...
		for (int dimension = 0;  dimension <= 2;  dimension++)
		{
			...
			foreach (string fieldKey in fieldKeys)
			{
				...
				AppendResetCustomVariableMethod(sb, fieldKey, variableType, dimension, isEnum);
				shouldAppendSpace = true;
				didAppendSpaceForSection = true;
				isFirstSection = false;
			}
	...
	static void AppendCustomVariable(StringBuilder sb, string type, string name, bool isEnum)
	{
		sb.Append("".PadLeft(GDECodeGenConstants.IndentLevel2));

		if (isEnum)
			sb.AppendFormat(GDECodeGenConstants.EnumVariableFormat, type, name);
		else
	...
	static void AppendCustomListVariable(StringBuilder sb, string type, string name, int dimension, bool isEnum)
	{
		...
		if (dimension == 1)
			sb.AppendFormat(GDECodeGenConstants.OneDListVariableFormat, type, name, methodType);
		else
			sb.AppendFormat(GDECodeGenConstants.TwoDListVariableFormat, type, name, methodType + GDECodeGenConstants.TwoDListSuffix);
	...
	static void AppendSaveToDictVariable(StringBuilder sb, string schema, string field, int dimension)
	{
		string prefix = string.Empty;
		sb.Append("".PadLeft(GDECodeGenConstants.IndentLevel3));

		if (GDEItemManager.IsEnum(schema, field))
			prefix = "Enum";

		sb.AppendFormat(GDECodeGenConstants.SaveToDictVarFormat, field, prefix);
	...
	static void AppendLoadCustomVariable(StringBuilder sb, string type, string name, bool isEnum)
	{
		sb.Append("".PadLeft(GDECodeGenConstants.IndentLevel4));
		if (!isEnum)
			sb.AppendFormat(GDECodeGenConstants.LoadCustomVariableFormat, type, name);
		else
			sb.AppendFormat(GDECodeGenConstants.LoadEnumVariableFormat, type, name);
		sb.Append("\n");
	}

	static void AppendLoadFromSavedCustomVariable(StringBuilder sb, string type, string name, bool isEnum)
	{
		sb.Append("".PadLeft(GDECodeGenConstants.IndentLevel3));
	...
	static void AppendLoadListVariable(StringBuilder sb, string type, string name, int dimension, bool isEnum)
	{
		sb.Append("".PadLeft(GDECodeGenConstants.IndentLevel4));
		string format = GDECodeGenConstants.LoadVariableListFormat;
		string prefix = string.Empty;

		if (isEnum)
		{
			type = "Enum";
			format = GDECodeGenConstants.LoadEnumListFormat;
		}

		if (dimension == 2)
			prefix = GDECodeGenConstants.TwoDListSuffix;
		
		sb.AppendFormat(format, type + prefix, name);
		sb.Append("\n");
	}

	static void AppendLoadSavedListVariable(StringBuilder sb, string type, string name, int dimension, bool isEnum)
	{
		string formattedType = type;
		if (GDEItemManager.IsUnityType(type))
			formattedType = "UnityObject";
	...
	static void AppendResetCustomVariableMethod(StringBuilder sb, string name, string type, int dimension, bool isEnum)
	{
		sb.Append("".PadLeft(GDECodeGenConstants.IndentLevel2));
		...
		{
			if (dimension == 0)
				sb.AppendFormat(GDECodeGenConstants.ResetEnumFormat, type, name);
			else if (dimension == 1)
				sb.AppendFormat(GDECodeGenConstants.ResetEnum1DListFormat, type, name);
			else
				sb.AppendFormat(GDECodeGenConstants.ResetEnum2DListFormat, type + GDECodeGenConstants.TwoDListSuffix, name);
		}
		sb.Append("\n");
	}
	#endregion
