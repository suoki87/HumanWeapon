...
public class GDESaveHook : UnityEditor.AssetModificationProcessor
{
	...
	{
		if (GDEItemManager.ItemsNeedSave || GDEItemManager.SchemasNeedSave || GDEItemManager.ModDataNeedSave)
			GDEItemManager.Save();
		return paths;
	}
}
...
public abstract class GDEManagerWindowBase : EditorWindow
{
	...
	protected virtual void Update()
	{
		...
		{
			if (highlightColorEnd.Equals(GDEConstants.NewHighlightStart.ToColor32()))
			{
	...
	protected virtual void OnGUI()
	{
		...
		drawTop = EditorGUI.Foldout(new Rect(drawHelper.CurrentLinePosition, drawHelper.TopOfLine(), size.x, size.y), drawTop, string.Empty);
		...
		float boxWidth = rateLinkSize.x;
		float leftBoundary = drawHelper.FullWindowWidth() - boxWidth - 10f;
		float topOfSection = drawHelper.TopOfLine();
        ...
        // Draw GDE Online Link
        content.text = GDEConstants.TryGDEOLink;
        drawHelper.TryGetCachedSize(GDEConstants.SizeTryGDEOLinkKey, content, linkStyle, out rateLinkSize);
        boxWidth = rateLinkSize.x;
        leftBoundary = drawHelper.FullWindowWidth() - boxWidth - 10f;
        topOfSection = drawHelper.TopOfLine();
        if (GUI.Button(new Rect(leftBoundary + (boxWidth - rateLinkSize.x) / 2f + 5.5f, topOfSection + 15f, rateLinkSize.x, rateLinkSize.y), content, linkStyle))
        {
	...
	protected virtual void DrawHeader()
	{
		...
		size.x = 160;
		drawHelper.NewLine(buttonHeightMultiplier + .1f);
		if (GUI.Button(new Rect(drawHelper.CurrentLinePosition, drawHelper.TopOfLine(), size.x, drawHelper.StandardHeight()*buttonHeightMultiplier), GDEConstants.LoadGDEOBtn, loadButtonStyle))
		{
			Debug.Log("Downloading Data from GDE Online...");
			GDEDataManager.GDEOInit();
			GDEDataManager.masterDataPath = GDESettings.Instance.DataFilePath;
	...
	{
		GUI.Box(new Rect(left - 1f, top, 2f, height), string.Empty, rateBoxStyle);
		GUI.Box(new Rect(left + 1, top, width, 2f), string.Empty, rateBoxStyle);
	...
	protected virtual Vector2 DrawExpandCollapseAllFoldout(string[] forKeys, string headerText)
	{
		Vector2 headerSize = drawHelper.DrawSubHeader(headerText, headerColor, GDEConstants.SizeListSubHeaderKey);
		Vector2 rightOfHeader = new Vector2(headerSize.x, drawHelper.TopOfLine() - headerSize.y - drawHelper.LineHeight / 4f);
	...
	protected virtual void DrawAudio(string fieldKey, string fieldName, Dictionary<string, object> data, string label)
	{
		try
		{
			...
			if (newClip != (currentValue as AudioClip))
			{
				data[key] = newClip;
				SetNeedToSave(true);
			}
	...
	protected virtual Dictionary<string, Dictionary<string, object>> GetEntriesToDraw(Dictionary<string, Dictionary<string, object>> source)
	{
	...
	protected virtual bool ResizeList(IList list, int size, object defaultValue)
	{
		bool result = false;
	...
	protected virtual void Load(bool forceLoad = false)
	{
		GDEItemManager.Load(forceLoad);
