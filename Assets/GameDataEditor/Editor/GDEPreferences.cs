...
public class GDEPreferences : SettingsProvider
{
	...
	public GDEPreferences(string path, SettingsScope scope) : base(path, scope) {}
	...
	[SettingsProvider]
	public static SettingsProvider CreateGDEPreferencesProvider()
	{
		var provider = new GDEPreferences(GDEConstants.PreferencesHeader, SettingsScope.User);
		return provider;
	}
	...
	public override void OnGUI(string searchContext)
	{
		...
		content.text = GDEConstants.VersionLbl;
		drawHelper.TryGetCachedSize(GDEConstants.VersionLbl, content, EditorStyles.label, out size);
		GUILayout.Label(content.text, GUILayout.Width(size.x), GUILayout.Height(size.y));
