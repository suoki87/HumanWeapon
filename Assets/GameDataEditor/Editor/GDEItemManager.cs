...
public class GDEItemManager
{
    ...
    public static bool ModDataNeedSave;
    ...
    static Dictionary<string, List<string>> ListByFieldName = new Dictionary<string, List<string>>();
    ...
    static string[] BuildSchemaFilterKeyArray()
    {
        ...
        temp.Sort();
        temp.Insert(0, GDEConstants.AllLbl);
        return temp.ToArray();
    }
    ...
    static string[] BuildBasicTypeStringArray()
    {
        ...
        for (int index = 0; index < basicTypeArray.Length;  index++)
            basicTypeArray[index] = BasicFieldTypes[index].EnumToString();
    ...
    public static string GetSchemaForItem(string itemKey)
    {
        ...
        if (AllItems.TryGetValue(itemKey, out itemData))
            itemData.TryGetString(GDMConstants.SchemaKey, out schema);
    ...
    public static List<string> ItemFieldKeysOfType(string itemKey, string fieldType, Dictionary<string, Dictionary<string, object>> data, int listDimension)
    {
        return FieldKeysOfType(itemKey, fieldType, data, listDimension);
    }
    ...
    public static List<string> ItemCustomFieldKeys(string itemKey, Dictionary<string, Dictionary<string, object>> data, int listDimension)
    {
        return CustomFieldKeys(itemKey, data, listDimension);
    }
    ...
    static Dictionary<int, List<string>> fieldKeysOfTypeCache = new Dictionary<int, List<string>>();
    ...
    {
        int hash = new {key, fieldType, listDimension} .GetHashCode();
        if (fieldKeysOfTypeCache.ContainsKey(hash))
            return fieldKeysOfTypeCache[hash];
        else
        {
            ...
            if (dict.TryGetValue(key, out data))
            {
                foreach (KeyValuePair<string, object> field in data)
                {
                    ...
                    isListKey = fieldName.ListKey();

                    data.TryGetInt(isListKey, out currentListDimension);

                    if (field.Value.ToString().Lower().Equals(fieldType.Lower()) && (listDimension == currentListDimension || listDimension == -1))
                        fieldKeys.Add(fieldName);
                }
            }

            fieldKeysOfTypeCache.Add(hash, fieldKeys);

            return fieldKeys;
        }
    }

    static Dictionary<int, List<string>> customFieldKeys = new Dictionary<int, List<string>>();
    // listDimension = -1, returns all keys matching type
    // listDimension = 0, returns single keys matching type
    // listDimension = 1, returns List<> keys matching type
    // listDimension = 2, returns 2D List keys matching type
    ...
    {
        int hash = new {key, listDimension} .GetHashCode();
        if (customFieldKeys.ContainsKey(hash))
            return customFieldKeys[hash];
        else
        {
            List<string> fieldKeys = new List<string>();
            Dictionary<string, object> data;
            ...
            customFieldKeys.Add(hash, fieldKeys);
    ...
    public static void LoadRuntimeData()
    {
        ...
        {
            string contents = File.ReadAllText(GDESettings.Instance.DataFilePath);
            GDEDataManager.InitFromText(contents);

            ModDataNeedSave = false;
        }
    }
    ...
    {
        // Load the enum types using reflection
        // Only load public enums defined in GameDataEditor namespace
        var assemblies = AppDomain.CurrentDomain.GetAssemblies();
        var allEnums = assemblies.SelectMany(t => t.GetTypes())
                       .Where(t => t.IsEnum && t.Namespace == "GameDataEditor"
                              && t.IsPublic && !ShouldFilterEnum(t.Name))
                       .ToList();

        if (allEnums.Count > 0)
            EnumTypes = allEnums.Select(x => x.Name).ToArray();
        else
            EnumTypes = new string[1] { "None" };

        // Load the enum values of the enums found above
        GDEItemManager.EnumValues.Clear();
        foreach (var item in allEnums)
        {
            var converter = new EnumConverter(item);
            var itemValues = converter.GetStandardValues();
            var valueStrings = new List<string>();

            foreach (var val in itemValues)
                valueStrings.Add(val.ToString());

            EnumValues.Add(item.Name, valueStrings);
        }
    }
    ...
    public static void Save()
    {
        try
        {
            if (ItemsNeedSave || SchemasNeedSave)
            {
                ...
                string relativePath = DataFilePath.Replace(Environment.CurrentDirectory, string.Empty).TrimStart(Path.DirectorySeparatorChar);
    ...
    public static bool CloneItem(string key, out string newKey, out string error)
    {
        ...
        if (AllItems.ContainsKey(key))
        {
            ...
            error = "GDE Editor cannot clone runtime items.";
    ...
    public static bool AddBasicFieldToSchema(BasicFieldType type, string schemaKey, Dictionary<string, object> schemaData, string newFieldName, out string error, bool isList = false, bool is2DList = false, object defaultValue = null)
    {
        ...
        string typeKey = newFieldName.TypeKey();
        error = string.Empty;
        ...
        if (result)
        {
            if (isList)
            {
                if (IsUnityType(type))
                {
                    ...
                    else if (type.Equals(BasicFieldType.VideoClip))
                        defaultValue = new List<VideoClip>();
                }
                else
                    defaultValue = new List<object>();

                schemaData.Add(newFieldName.ListKey(), 1);
            }
            else if (is2DList)
            {
                if (IsUnityType(type))
                {
                    if (type.Equals(BasicFieldType.GameObject))
                        ...
                        defaultValue = new List<List<VideoClip>>();
                }
                else
                    defaultValue = new List<object>();

                schemaData.Add(newFieldName.ListKey(), 2);
            }
    ...
    public static bool AddCustomFieldToSchema(string customType, string schemaKey, Dictionary<string, object> schemaData, string newFieldName, bool isList, bool is2DList, out string error)
    {
        ...
        if (result)
        {
            if (isList)
            {
                ...
                schemaData.Add(newFieldName.TypeKey(), customType);
                schemaData.Add(newFieldName.ListKey(), 1);
            }
            else if (is2DList)
            {
    ...
    public static void RemoveFieldFromSchema(string schemaKey, Dictionary<string, object> schemaData, string deletedFieldKey, bool deleteFromItem = true)
    {
        ...
        schemaData.Remove(deletedFieldKey.TypeKey());
        schemaData.Remove(deletedFieldKey.ListKey());
    ...
    static void AddBasicFieldToItems(BasicFieldType type, string schemaKey, string newFieldName, bool isList, bool is2DList, object defaultValue)
    {
        ...
        foreach (string itemKey in itemKeys)
        {
            if (AllItems.TryGetValue(itemKey, out itemData))
            {
                ...
                itemData.Add(newFieldName.TypeKey(), type);
                itemData.Add(newFieldName.ListKey(), listDimension);
                ItemsNeedSave = true;
                ClearCaches();
            }
        }
        ...
        foreach (string itemKey in itemKeys)
        {
            if (AllItems.TryGetValue(itemKey, out itemData))
            {
                if (isList)
                {
                    ...
                    itemData.Add(newFieldName.TypeKey(), customType);
                    itemData.Add(newFieldName.ListKey(), 1);
                }
                else if (is2DList)
                {
                    itemData.Add(newFieldName, new List<object>());
                    itemData.Add(newFieldName.TypeKey(), customType);
                    itemData.Add(newFieldName.ListKey(), 2);
                }
                else
                {
    ...
    static void RemoveFieldFromItems(string schemaKey, string deleteFieldName)
    {
        ...
        foreach (string itemKey in itemKeys)
        {
            if (AllItems.TryGetValue(itemKey, out itemData))
            {
                ...
                itemData.Remove(deleteFieldName.TypeKey());
                itemData.Remove(deleteFieldName.ListKey());
                    ...
                    foreach (string curSchemaKey in AllSchemas.Keys)
                    {
                        ...
                        if (fieldsOfSchemaType.Count > 0)
                        {
                            ...
                            foreach (string schemaFieldKey in fieldsOfSchemaType)
                                curSchemaData.TryAddOrUpdateValue(schemaFieldKey.TypeKey(), newSchemaKey);
                        }
                    }
                    ...
                    foreach (string curItemKey in AllItems.Keys)
                    {
                        ...
                        if (fieldsOfSchemaType.Count > 0)
                        {
                            ...
                            foreach (string itemFieldKey in fieldsOfSchemaType)
                                curItemData.TryAddOrUpdateValue(itemFieldKey.TypeKey(), newSchemaKey);
                        }
                    }
                }
                else
    ...
    static void RenameField(string oldFieldKey, string newFieldKey, Dictionary<string, object> data)
    {
        ...
        string oldKey = oldFieldKey.TypeKey();
        string newKey = newFieldKey.TypeKey();
        ...
        oldKey = oldFieldKey.ListKey();
        newKey = newFieldKey.ListKey();
        if (data.TryGetValue(oldKey, out value))
        {
            data.Add(newKey, value);
            data.Remove(oldKey);
    ...
    // The Schema Mgr and the Item Mgr need access to these
    // so I'll have the GDEItemManager keep them
    public static string[] EnumTypes = new string[] { "None" };
    public static Dictionary<string, List<string>> EnumValues = new Dictionary<string, List<string>>();

    public static void ClearCaches()
    {
        fieldKeysOfTypeCache.Clear();
        customFieldKeys.Clear();
    }

    public static string GetTypeForField(string schema, string itemKey)
    {
        object type = null;
        Dictionary<string, object> schemaData; 
        
        if (AllSchemas.TryGetValue(schema, out schemaData))
            if (schemaData.TryGetValue(itemKey.GDETypeKey(), out type))
    ...
    public static object GetDefaultRuntimeValueForType(BasicFieldType type)
    {
        if (type.IsSet(BasicFieldType.Bool))
            return false;

        if (type.IsSet(BasicFieldType.Float))
            return 0f;

        if (type.IsSet(BasicFieldType.Int))
            return 0;

        if (type.IsSet(BasicFieldType.String))
            return string.Empty;

        if (type.IsSet(BasicFieldType.Vector2))
            return new Vector2();

        if (type.IsSet(BasicFieldType.Vector3))
            return new Vector3();

        if (type.IsSet(BasicFieldType.Vector4))
            return new Vector4();

        if (type.IsSet(BasicFieldType.Color))
            return new Color();
    ...
    public static Dictionary<string, object> GetJsonReadyDataCopy()
    {
        ...
        foreach (var key in keys)
        {
            ...
            allData.Add(key.SchemaKey(), schemaData);
        }
    ...
    public static void LoadSpecialTypes(Dictionary<string, object> data)
    {
        ...
        foreach (string key in keys)
        {
            ...
            if (listDimension == 0)
            {
                if (fieldType.Equals(BasicFieldType.GameObject.EnumToString()))
                {
                    GameObject go;
                    data.TryGetGameObject(fieldName, out go);
                    data.TryAddOrUpdateValue(fieldName, go);
                }
                else if (fieldType.Equals(BasicFieldType.Texture2D.EnumToString()))
                {
                    Texture2D tex;
                    data.TryGetTexture2D(fieldName, out tex);
                    data.TryAddOrUpdateValue(fieldName, tex);
                }
                else if (fieldType.Equals(BasicFieldType.Material.EnumToString()))
                {
                    Material mat;
                    data.TryGetMaterial(fieldName, out mat);
                    data.TryAddOrUpdateValue(fieldName, mat);
                }
                else if (fieldType.Equals(BasicFieldType.AudioClip.EnumToString()))
                {
                    AudioClip aud;
                    data.TryGetAudioClip(fieldName, out aud);
                    data.TryAddOrUpdateValue(fieldName, aud);
                }
                else if (fieldType.Equals(BasicFieldType.VideoClip.EnumToString()))
                {
                    VideoClip vid;
                    data.TryGetVideoClip(fieldName, out vid);
                    data.TryAddOrUpdateValue(fieldName, vid);
                }
            }
            else if (listDimension == 1)
            {
                ...
                else if (fieldType.Equals(BasicFieldType.VideoClip.EnumToString()))
                {
                    ...
                    data.TryGetVideoClipList(fieldName, out vidList);
                    data.TryAddOrUpdateValue(fieldName, vidList);
                }
            }
            else if (listDimension == 2)
            {
                ...
                {
                    List<List<VideoClip>> vidTwoDList;
                    data.TryGetVideoClipTwoDList(fieldName, out vidTwoDList);
                    data.TryAddOrUpdateValue(fieldName, vidTwoDList);
                }
            }
        }
    }
    ...
    public static string GetJsonRepresentation(object value)
    {
        if (value != null && value.DerivesFromUnityObject())
            return AssetDatabase.GetAssetPath(value as UnityEngine.Object).StripAssetPath();

        return string.Empty;
    }

    static Dictionary<BasicFieldType, string> enumStringCache = new Dictionary<BasicFieldType, string>();
    public static string GetVariableTypeFor(BasicFieldType type)
    {
        if (enumStringCache.ContainsKey(type))
            return enumStringCache[type];
        else
        {
            string result = type.EnumToString();
            ...
            enumStringCache.Add(type, result);
    ...
    public static bool IsUnityType(BasicFieldType type)
    {
        return type.Equals(BasicFieldType.GameObject) ||
               ...
               type.Equals(BasicFieldType.VideoClip);
    }

    public static bool IsUnityType(string type)
    {
        return type.Equals(BasicFieldType.GameObject.EnumToString()) ||
               type.Equals(BasicFieldType.Texture2D.EnumToString()) ||
               type.Equals(BasicFieldType.Material.EnumToString()) ||
               type.Equals(BasicFieldType.AudioClip.EnumToString()) ||
               type.Equals(BasicFieldType.VideoClip.EnumToString());
    }

    static List<string> invalidEnums = new List<string> { "BasicFieldType", "ImportExportType", "GDEImportView" };
    public static bool ShouldFilterEnum(string name)
    {
        return invalidEnums.Contains(name);
    }

    public static bool IsEnum(string schema, string itemKey)
    {
        var type = GetTypeForField(schema, itemKey);
