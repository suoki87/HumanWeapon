...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip(GDMConstants.GetVideoClipListCustomActionTooltip)]
	public class GDEGetCustomVideoClipArray : GDEActionBase
	{   
		[UIHint(UIHint.FsmString)]
		[Tooltip(GDMConstants.VideoClipListCustomFieldTooltip)]
		...
		public override void OnEnter()
		{
			try
			{
				...
				if (GDEDataManager.DataDictionary.ContainsKey(ItemName.Value))
				{
					...
					customData.TryGetVideoClipList(CustomField.Value, out val);
				}
				else
				{
					// New item case
					...
					if (GDEDataManager.Get(customKey, out data))
						data.TryGetVideoClipList(CustomField.Value, out val);
				}
				
				// Override from saved data if it exists
				val = GDEDataManager.GetUnityObjectList<VideoClip>(customKey, CustomField.Value, val);
