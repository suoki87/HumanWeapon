...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip(GDMConstants.GetVec2ListCustomActionTooltip)]
	public class GDEGetCustomVector2Array : GDEActionBase
	{   
		...
		public override void OnEnter()
		{
			try
			{
				...
				if (GDEDataManager.DataDictionary.ContainsKey(ItemName.Value))
				{
					...
					customData.TryGetVector2List(CustomField.Value, out val);
				}
				else
				{
					// New item case
					...
					if (GDEDataManager.Get(customKey, out data))
						data.TryGetVector2List(CustomField.Value, out val);
				}
				
				// Override from saved data if it exists
				val = GDEDataManager.GetVector2List(customKey, CustomField.Value, val);
