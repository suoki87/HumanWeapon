...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip(GDMConstants.GetFloatListCustomActionTooltip)]
	public class GDEGetCustomFloatArray : GDEActionBase
	{   
		...
		public override void OnEnter()
		{
			try
			{
				...
				if (GDEDataManager.DataDictionary.ContainsKey(ItemName.Value))
				{
					...
					customData.TryGetFloatList(CustomField.Value, out val);
				}
				else
				{
					// New item case
					...
					if (GDEDataManager.Get(customKey, out data))
						data.TryGetFloatList(CustomField.Value, out val);
				}
				
				// Override from saved data if it exists
				val = GDEDataManager.GetFloatList(customKey, CustomField.Value, val);
