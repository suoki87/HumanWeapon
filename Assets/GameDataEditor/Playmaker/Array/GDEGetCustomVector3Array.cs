...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip(GDMConstants.GetVec3ListCustomActionTooltip)]
	public class GDEGetCustomVector3Array : GDEActionBase
	{   
		[UIHint(UIHint.FsmString)]
		[Tooltip(GDMConstants.Vec3ListCustomFieldTooltip)]
		...
		public override void OnEnter()
		{
			try
			{
				...
				if (GDEDataManager.DataDictionary.ContainsKey(ItemName.Value))
				{
					...
					customData.TryGetVector3List(CustomField.Value, out val);
				}
				else
				{
					// New item case
					...
					if (GDEDataManager.Get(customKey, out data))
						data.TryGetVector3List(CustomField.Value, out val);
				}
				
				// Override from saved data if it exists
				val = GDEDataManager.GetVector3List(customKey, CustomField.Value, val);
