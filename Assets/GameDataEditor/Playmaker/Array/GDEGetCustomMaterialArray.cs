...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip(GDMConstants.GetMaterialListCustomActionTooltip)]
	public class GDEGetCustomMaterialArray : GDEActionBase
	{   
		[UIHint(UIHint.FsmString)]
		[Tooltip(GDMConstants.MaterialListCustomFieldTooltip)]
		...
		public override void OnEnter()
		{
			try
			{
				...
				if (GDEDataManager.DataDictionary.ContainsKey(ItemName.Value))
				{
					...
					customData.TryGetMaterialList(CustomField.Value, out val);
				}
				else
				{
					// New item case
					...
					if (GDEDataManager.Get(customKey, out data))
						data.TryGetMaterialList(CustomField.Value, out val);
				}
				
				// Override from saved data if it exists
				val = GDEDataManager.GetUnityObjectList<Material>(customKey, CustomField.Value, val);
